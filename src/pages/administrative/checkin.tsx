...
export default function Page() {
  ...
  // Rango según el modo:
  //  - Sin paciente (tablero del día): SOLO los turnos de hoy, todos los pacientes.
  //  - Con un paciente elegido: rango amplio (el mes) para ver todos sus turnos y
  //    marcarle la llegada aunque no sean de hoy (útil para pruebas/demo).
  const appointmentsParams: GetAppointmentsRequest = {
    since: format(new Date(), 'yyyy-MM-dd 00:00:00'),
    until: hasPatient
      ? format(addDays(new Date(), 30), 'yyyy-MM-dd 00:00:00')
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 rounded-lg bg-muted px-3 py-2 text-xs">
            ...
            <span className="text-muted-foreground">{hasPatient ? 'del paciente' : 'total del día'}</span>
          </div>
          <button
            type="button"
