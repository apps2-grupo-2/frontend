...
export const startAppointment = async (id: number): Promise<StartAppointmentResponse> => {
  const response = await axios.patch<StartAppointmentResponse>(`${ENV.BASE_URL}/appointments/${id}/start`);
  return response.data;
};
